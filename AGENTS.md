...
- You should assume that all toot services are running. To verify, you can check the `backend/log` directory to
...
**toot** (formerly Danswer) is an open-source Gen-AI and Enterprise Search platform that connects to company documents, apps, and people. It features a modular architecture with both Community Edition (MIT licensed) and Enterprise Edition offerings.


### Background Workers (Celery)

toot uses Celery for asynchronous task processing with multiple specialized workers:
...
toot supports two deployment modes for background workers, controlled by the `USE_LIGHTWEIGHT_BACKGROUND_WORKER` environment variable:
...
There are 4 main types of tests within toot:

### Unit Tests
These should not assume any toot/external services are available to be called.
...
These tests assume that all external dependencies of toot are available and callable (e.g. Postgres, Redis, 
MinIO/S3, Vespa are running + OpenAI can be called + any request to the internet is fine + etc.).

However, the actual toot containers are not running and with these tests we call the function to test directly.
...
### Integration Tests
Standard integration tests. Every test in `backend/tests/integration` runs against a real toot deployment. We cannot 
...
These tests are an even more complete version of the Integration Tests mentioned above. Has all services of toot 
...
to logs via the `backend/log/<service_name>_debug.log` file. All toot services (api_server, web_server, celery_X)
