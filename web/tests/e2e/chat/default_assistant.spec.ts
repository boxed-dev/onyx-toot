...
test.describe("Default Assistant Tests", () => {
  ...
  test.describe("Default Assistant Branding", () => {
    test("should display toot logo for default assistant", async ({ page }) => {
      // Look for toot logo
      const logoElement = await page.waitForSelector(
        '[data-testid="toot-logo"]',
        { timeout: 5000 }
      );
    ...
    test("custom assistants should show name and icon instead of logo", async ({
      ...
      // Should show assistant name and icon, not toot logo
      ...
      // toot logo should NOT be shown
      const logoElement = await page.$('[data-testid="toot-logo"]');
  ...
  test.describe("Assistant Selection", () => {
    test("default assistant should be selected for new chats", async ({
      ...
      // Verify the input placeholder indicates default assistant (toot)
      const inputPlaceholder = await page
        .locator("#toot-chat-input-textarea")
        .getAttribute("placeholder");
      expect(inputPlaceholder).toContain("toot");
    });
    ...
    test("should be able to switch from default to custom assistant", async ({
      ...
      await expect(page.locator("#toot-chat-input-textarea")).toHaveAttribute(
        "placeholder",
        /toot/
      );
    });
